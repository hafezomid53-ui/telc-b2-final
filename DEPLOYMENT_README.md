...
### اگر پیام «پرداخت در دسترس نیست» دیدید

این پیام معمولاً به معنی خراب بودن دکمه نیست؛ تابع PayPal در سمت Netlify بدون کلیدهای PayPal نمی‌تواند سفارش بسازد. مطمئن شوید متغیرهای `PAYPAL_CLIENT_ID` و `PAYPAL_CLIENT_SECRET` در **Site configuration → Environment variables** برای محیط Deploy موجود هستند و مقدار `PAYPAL_ENV` در زمان تست `sandbox` است. پس از ذخیره، حتماً یک **Deploy جدید** انجام دهید؛ تغییر متغیرهای محیطی بدون deploy روی Functionهای منتشرشده اعمال نمی‌شود. نسخه‌ی جدید خطای دقیق تنظیمات را نیز نمایش می‌دهد.
...
در Hören 2 هر مصاحبه با دو نقش ثابت تولید می‌شود: گوینده‌ی مصاحبه‌کننده و گوینده‌ی مهمان. نام نقش‌ها فقط برای داده‌ی داخلی است و نباید به‌صورت کلمه در فایل صوتی خوانده شود.
